...
use contact_normal::fit_contact_normal_lr;
...
type MixtureCellMap = HashMap<u32, MixtureCell, FxU32BuildHasher>;
...
pub struct Grid {
    ...
    /// Two-phase mixture coupling field — see `MixtureCell` doc. Empty for every
    /// scene that never uses `WithMixturePhase`, the same zero-cost property
...
impl Grid {
    ...
    fn project_mixture_incompressibility(&mut self, cell_width: f32, pressure_iterations: u32) {
        ...
        for &idx in &self.mixture_dirty {
            let (Some(&a_s), Some(&a_f)) = (alpha_s.get(&idx), alpha_f.get(&idx)) else {
                continue;
            };
            let pos = self.idx_to_pos(idx);
            let p_r = p_or_zero(&pressure, pos + IVec2::new(1, 0));
