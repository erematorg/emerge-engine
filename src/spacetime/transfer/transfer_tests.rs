//! Test suite for `transfer.rs` (P2G/G2P forward passes + differentiable
//! adjoints) -- split out into its own file (was ~1280 of the parent file's
//! ~1950 lines), same pattern as `gpu/solver/device_lost_tests.rs` and
//! `materials/elastic/elastic_tests.rs`. Pure mechanical line-range
//! extraction, not retyped, to eliminate transcription risk in adjoint math
//! this precise (every VJP here is verified against central-difference
//! numerical gradients -- exactly the code where a silent copy error would
