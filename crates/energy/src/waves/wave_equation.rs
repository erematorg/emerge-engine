...
// Math is pure Rust. Reflect derive is required here only because WaveEquationComponent
// (below) holds this struct and derives Reflect — Bevy requires all fields to implement it.
// To remove the Bevy dep from this struct, WaveEquationComponent would need #[reflect(ignore)]
// on the solver field. Deferred — not worth the disruption now.
