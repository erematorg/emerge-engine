...
use crate::pairwise::{
    DeterminismConfig, for_each_neighbor_candidate, is_forward_entity_pair,
...
impl Default for ThermalDiagnostics {
    fn default() -> Self {
        Self {
            enabled: true,
...
pub(crate) fn compute_fourier_conduction(
    ...
    config: Res<ThermalConductionParams>,
    ...
    // Apply temperature changes — direct mutation, no deferred commands.
    for (&entity, &delta) in &temp_changes {
        if let Ok((_, _, mut temp, _, _, _)) = entities.get_mut(entity) {
            temp.value = (temp.value + delta).max(0.0);
        }
    }
...
impl Plugin for ThermalSystemPlugin {
    fn build(&self, app: &mut App) {
        app.init_resource::<ThermalConductionParams>()
            .init_resource::<ThermalDiagnostics>()
            .register_type::<ThermalConductionParams>()
            .register_type::<ThermalDiagnostics>()
            ...
            // Physics runs at fixed rate — dt is bounded, CFL holds.
            .add_systems(
                FixedUpdate,
                (compute_fourier_conduction, sync_thermal_energy).chain(),
            )
            // Diagnostics are presentation-only, variable rate is fine.
            .add_systems(Update, check_thermal_sanity_realtime);
    }
}
