...
pub fn calculate_field_interactions(
    mut field_interaction_events: MessageWriter<EMInteractionEvent>,
    ...
    while let Some([(entity_a, field_a), (entity_b, field_b)]) = electric_pairs.fetch_next() {
        ...
        if interaction_strength > f32::EPSILON {
            field_interaction_events.write(EMInteractionEvent {
                source: entity_a,
                target: entity_b,
...
impl Plugin for ElectromagneticFieldPlugin {
    fn build(&self, app: &mut App) {
        app
            ...
            .add_message::<EMInteractionEvent>()
