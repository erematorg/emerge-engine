...
///   GRID=64, DT=0.1, GRAVITY=-0.3 cell/step^2. Fall from y=48 to floor ~46 cells in ~55 steps.
use emerge::{
    BinghamFluidMaterial, DruckerPragerMaterial, FrictionBoundary, MuIRheologyMaterial,
    NeoHookeanMaterial, NewtonianFluidMaterial, RankineMaterial, SimConfig, Simulation,
    SlipBoundary, SpawnRegion, StomakhinMaterial, ViscoelasticMaterial, VonMisesMaterial,
    lame_from_young,
};
use glam::{IVec2, Vec2};
...
fn spawn_at(center: Vec2, size: IVec2, spacing: f32, material_id: u32) -> SpawnRegion {
    SpawnRegion {
        spacing,
        box_size: size,
        box_center: center,
...
fn center_spawn(material_id: u32) -> SpawnRegion {
...
fn run_basic() {
    ...
    println!("  grid={GRID}x{GRID}, dt={DT}, gravity={GRAVITY}");
    println!("  spawn y=48, floor at y~2, expected fall ~46 cells\n");

    type Case = (&'static str, Box<dyn Fn() -> Simulation>);
    let cases: &[Case] = &[
        (
            "water (Newtonian)",
            Box::new(|| {
        ...
        (
            "mud (Bingham)",
            Box::new(|| {
                let m = BinghamFluidMaterial::high_yield(1500.0, 1.0e4);
                Simulation::new(config(), center_spawn(0))
                    .with_default_material(Box::new(m))
                    .with_boundary(Box::new(SlipBoundary::new(2)))
            }),
        ...
        (
            "snow",
            Box::new(|| {
                let m = StomakhinMaterial::from_young_modulus(1.4e5, 0.2);
                Simulation::new(config(), center_spawn(0))
                    .with_default_material(Box::new(m))
                    .with_boundary(Box::new(SlipBoundary::new(2)))
            }),
        ...
        (
            "sand (DP)",
            Box::new(|| {
                let m = DruckerPragerMaterial::from_young_modulus(1e5, 0.2);
                Simulation::new(config(), center_spawn(0))
                    .with_default_material(Box::new(m))
                    .with_boundary(Box::new(SlipBoundary::new(2)))
            }),
        ...
        (
            "rankine (rock)",
            Box::new(|| {
                let m = RankineMaterial::stiff_brittle(5e5, 0.25);
                Simulation::new(config(), center_spawn(0))
                    .with_default_material(Box::new(m))
                    .with_boundary(Box::new(SlipBoundary::new(2)))
            }),
        ...
        (
            "viscoelastic",
            Box::new(|| {
                let m = ViscoelasticMaterial::near_incompressible(5.0e4, 10.0);
                Simulation::new(config(), center_spawn(0))
                    .with_default_material(Box::new(m))
                    .with_boundary(Box::new(SlipBoundary::new(2)))
            }),
        ...
        (
            "sand mu(I)",
            Box::new(|| {
                let m = MuIRheologyMaterial::small_grain(1e5, 0.2);
                Simulation::new(config(), center_spawn(0))
                    .with_default_material(Box::new(m))
...
fn run_sweep() {
    ...
    for &e in &[1e3f32, 1e4, 5e4, 1e5, 5e5, 1e6] {
        let m = DruckerPragerMaterial::from_young_modulus(e, 0.2);
        let solver = Simulation::new(config(), center_spawn(0))
            .with_default_material(Box::new(m))
            .with_boundary(Box::new(SlipBoundary::new(2)));
        let r = Report::from_solver(format!("E={e:.0}"), 0, solver, 300);
...
fn run_perf() {
    ...
    println!("  Material: NeoHookean E=5e4. Grid=64x64. Steps=50 (warm-up 5 discarded).");
    ...
    for &n in &counts {
        ...
        let feasible = if ms_per_step < 16.0 { "[OK]" } else { "[X]" };
    ...
    println!("\n  Note: debug build. Release will be ~10-30x faster.");
...
fn run_scenarios() {
    ...
    println!("   Jelly blob (material 0) spawned above, sand floor (material 1) at bottom.");
    {
        ...
        let mut solver = Simulation::new(cfg, body_spawn)
        ...
        let _ = solver.add_body(floor_spawn);
        ...
        for step in 1..=300u64 {
            ...
            if step % 100 == 0 {
                let snap = solver.diagnostics_snapshot();
                let mat0 = solver.material_state(0);
    ...
    println!("   Tall column of sand collapses under gravity. Should form stable pile.");
    {
        ...
        let mut solver = Simulation::new(config(), sand_col)
        ...
        for step in 1..=500u64 {
            ...
            if step % 125 == 0 {
                ...
                println!(
                    ...
                    ms.centroid.y, ms.avg_speed, ms.avg_det_f, snap.non_finite_particle_values
                );
            }
        }
        let ms = solver.material_state(0);
        println!(
            "    wall={:.1}ms total  particles={}  final_centroid_y={:.2}",
            t0.elapsed().as_millis(),
            solver.particles().len(),
            ms.centroid.y
        );
        println!("    (dry sand angle of repose ~25-35 deg)");
    }

    // -- Scenario 4: Mixed material -- mud + elastic body ---------------------
    println!("\n-- Scenario 4: Elastic creature body in mud --");
    println!(
        "   Rigid-ish SpawnRegion (E=2e5) sinking into mud. Tests multi-material interaction."
    );
    {
        let cfg = SimConfig {
            max_substeps_per_step: 64,
        ...
        let neo = NeoHookeanMaterial::new(lb, ub);
        ...
        let _ = solver.add_body(mud_spawn);
