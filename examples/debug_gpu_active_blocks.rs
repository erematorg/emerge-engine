// Compare active-block-list/sort state between the stable 2-material case and
// the unstable 3-material (fluid-present) case, to check for a sparse-grid
// indexing bug rather than inferring purely from downstream J corruption.
//   cargo run --release --example debug_gpu_active_blocks --features gpu
...
fn run(with_water: bool) {
    let config = SimConfig {
        min_dt: 1.0e-5,
        max_substeps_per_step: 6000,
        ..SimConfig::earth(GRID_RES, 0.01, 0.05)
    };
    ...
    if with_water {
        ...
        registry.insert(WATER_ID, Box::new(creature_mat)); // creature at slot 1 instead
    }
    ...
    let creature_material_id = if with_water { CREATURE_ID } else { WATER_ID };
    let range = solver.spawn_region(SpawnRegion {
        spacing: 0.5,
        box_size: IVec2::new(6, 6),
        box_center: Vec2::new(GRID_RES as f32 * 0.5, 60.0),
        material_id: creature_material_id,
        precompute_initial_volumes: true,
        mass_override: Some(CREATURE_PROPS.particle_mass(0.5, &config)),
        ..SpawnRegion::for_sim(&config)
    });

    println!("--- with_water={with_water} ---");
    for i in 0..3 {
        solver.step_frame();
        solver.sync_particles_blocking();
        let active_count = solver.active_block_count_blocking();
        let js: Vec<f32> = solver.particles()[range.clone()]
            .iter()
            .map(|p| p.deformation_gradient.determinant())
            .collect();
        let max_j = js.iter().cloned().fold(0.0f32, f32::max);
        let min_j = js.iter().cloned().fold(f32::INFINITY, f32::min);
        println!(
            "step {i}: substeps={} active_blocks={} creature_min_J={:.4} creature_max_J={:.4}",
